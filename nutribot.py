...
# A utility function to integrate memory into QA logic
...
# Initialize conversational agent with tools and memory
...
print("✅ Enhanced Conversational Agent ready with memory integration!")
...
def chatbot_logic(message, input_type, uploaded_image, audio_path, health_condition, language_choice):
    ...
    if health_condition and health_condition.strip() != "":
        keywords = [
            "eat", "food", "meal", "diet", "drink", "nutrition",
            "breakfast", "lunch", "dinner", "snack",
            "وجبة", "فطور", "غداء", "عشاء", "أكل", "شرب", "افطار"
        ]
...
# Custom CSS styling for ChatGPT-like UI
gr_theme_css = """
<style>
@font-face {
    font-family: 'Dubai';
...
# Launch Gradio interface with ChatGPT-like design
with gr.Blocks(title="NutriBot: Smart Nutrition Assistant",
    ...
    <div style="text-align: center; margin-bottom: 20px;">
        <h1 style="color: #2e8b57; font-weight: 600;">NutriBot: Smart Nutrition Assistant for Personalized Dietary Support</h1>
        <p style="color: #666;">Type your question, speak, or upload food image to get healthy dietary suggestions</p>
    </div>
    """)

    # Options area (language and health condition)
    ...
    chatbot = gr.Chatbot(
        ...
        avatar_images=(
            "https://cdn-icons-png.flaticon.com/512/2715/2715229.png",  # User avatar
    ...
    # Multimedia inputs (hidden by default, shown when needed)
    with gr.Accordion("📎 Attach file (image or voice)", open=False):
        with gr.Row():
            with gr.Column():
                gr.HTML("<div style='text-align:center; font-size:20px;'>Upload Image</div>")
            ...
            with gr.Column():
                gr.HTML("<div style='text-align:center; font-size:20px;'>Record Voice</div>")
    ...
    def respond(message, chat_history, image, audio, health, lang_choice):
        try:
            ...
            if transcription_display:
                if input_type == "Image":
                    user_message = f"📷 Image: {transcription_display}"
                elif input_type == "Voice":
                    user_message = f"🎤 {transcription_display}"

            # Apply Arabic styling if language is Arabic
